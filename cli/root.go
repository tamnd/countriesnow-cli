// Package cli assembles the countriesnow command tree from the countriesnow
...
// NewApp assembles the kit application from the countriesnow domain. The
...
//
// To add a command, declare it in countriesnow/domain.go with kit.Handle and it
// appears here automatically. Reach for app.AddCommand only for a verb that does
// not fit the emit-records shape, the way version does below.
func NewApp() *kit.App {
	id := countriesnow.Domain{}.Info().Identity
	id.Version = Version

	app := kit.New(id)
	(countriesnow.Domain{}).Register(app)
