...
describes what users get. Pick a format with `-o`, or let countriesnow choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
countriesnow <command> -o table   # aligned columns for reading
countriesnow <command> -o jsonl   # one JSON object per line, for piping
countriesnow <command> -o json    # a single JSON array
countriesnow <command> -o csv     # spreadsheet friendly
countriesnow <command> -o tsv     # tab-separated
countriesnow <command> -o url     # just the URL column
countriesnow <command> -o raw     # the underlying bytes, unformatted
```
...
```bash
countriesnow <command> --fields id,title,url
```
...
```bash
countriesnow <command> --template '{{.URL}} {{.Title}}'
```
...
```bash
countriesnow <command>            # a table, because this is a terminal
countriesnow <command> | wc -l    # JSONL, because this is a pipe
```
